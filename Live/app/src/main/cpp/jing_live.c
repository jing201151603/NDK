#include "com_dxhj_live_jni_PushNative.h"
#include <pthread.h>
#include <malloc.h>
#include "queue.h"
#include "x264.h"
...
void add_rtmp_packet(RTMPPacket *packet) {
    ...
    pthread_cond_signal(&cond);//通知，根据阻塞变量唤醒阻塞线程，继续执行
...
void *push_thread(void *arg) {
    ...
    while (is_pushing) {
        //发送
        pthread_mutex_lock(&mutex);
        pthread_cond_wait(&cond, &mutex);//阻塞，等待条件变量被通知,收到通知后继续
        //取出队列中的RTMPPacket
        RTMPPacket *packet = queue_get_first();
        if (packet && is_pushing) {
...
JNIEXPORT void JNICALL Java_com_dxhj_live_jni_PushNative_startPush
    ...
    memcpy(rtmp_path, url_cstr, strlen(url_cstr));//复制
...
JNIEXPORT void JNICALL Java_com_dxhj_live_jni_PushNative_setVideoOptions
    ...
    //是否把SPS和PPS放入每一个关键帧
    //SPS Sequence Parameter Set 序列参数集，作用于一串连续的视频图像，即视频序列
    //PPS Picture Parameter Set 图像参数集，作用于视频序列中的一个或多个图像
    //为了提高图像的纠错能力
